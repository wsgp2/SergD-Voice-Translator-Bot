...
async def summarize_with_gpt(text: str, lang: str) -> str:
    ...
    try:
        ...
        # Улучшенный промпт для структурированной суммаризации с учетом особенностей Telegram Markdown
        system_prompt = """Ты - эксперт по созданию кратких и информативных резюме для Telegram. Создай структурированное резюме для предоставленного текста, строго соблюдая следующие правила форматирования Telegram:

1. Используй *звездочки* для выделения текста жирным шрифтом (вместо **двойных звездочек**)
2. Используй _подчеркивание_ для курсива (вместо *звездочек*)
3. Используй эмодзи в начале каждого раздела вместо символов # или ##
4. НЕ используй символ # для заголовков - вместо этого используй *жирный текст с эмодзи*
5. Для списков используй • или ○ или ◉ или другие символы с пробелом вместо дефисов (-)
6. Используй разделители "---" для отделения разделов

Структура резюме:
1. 📝 *Резюме* 🇷🇺 (или другой флаг страны в зависимости от языка) - заголовок в самом начале
2. Краткое введение (1-2 предложения) о чем этот текст
3. Разделы с эмодзи и *жирными заголовками*
4. Списки с символами • вместо дефисов
5. *Выделение важных фраз жирным шрифтом*
6. Выводы или ключевые мысли в конце
        ...
        with tempfile.NamedTemporaryFile(suffix='.ogg', delete=False) as temp_audio:
            ...
            try:
                ...
                if "summary" in result and result["summary"] and mode in [MODE_SUMMARIZE, MODE_BOTH]:
                    # Не добавляем заголовок, т.к. ChatGPT уже добавляет его в резюме
